...
- **Общая схема.** Интерфейс для доступа к ключевым понятиям аудита Office 365, таким как "тип записи", "время создания", "тип пользователя" и "действие", а также для предоставления основных измерений (например, ИД пользователя) и свойств, характерных для расположения (например, IP-адрес клиента) и продукта (например, ИД объекта). Эта схема обеспечивает согласованные и однородные представления, с помощью которых пользователи могут извлекать любые данные аудита Office 365 в нескольких представлениях верхнего уровня на основе соответствующих параметров. Кроме того, это фиксированная схема для всех источников данных, которая позволяет значительно сократить затраты на обучение. Общая схема создается основе данных о продуктах, например Exchange, SharePoint, Azure Active Directory, Yammer и OneDrive для бизнеса, каждый из которых разрабатывает отдельная группа. Группы разработчиков продуктов могут дополнять поле "ИД объекта", добавляя свойства конкретных продуктов.

- **Схема для конкретного продукта.** Создается на основе общей схемы, чтобы предоставить набор атрибутов для конкретных продуктов. Примеры: схема Sway, схема SharePoint, схема OneDrive для бизнеса и схема администрирования Exchange.

**Какой уровень следует использовать в конкретном варианте моделирования?**
Как правило, если данные доступны на более высоком уровне, не возвращайтесь на более низкий уровень. Другими словами, если требования к данным можно выполнить с помощью схемы для конкретного продукта, не нужно возвращаться к общей схеме. 

## <a name="office-365-management-api-schemas"></a>Схемы API управления Office 365

В этой статье описываются общая схема, а также все схемы для конкретного продукта. Доступные схемы приведены в таблице ниже. 

|Имя схемы|Описание|
|:-----|:-----|
|[Общая схема](#common-schema)|Представление, которое позволяет извлекать значения типа записи, ИД пользователя, IP-адреса клиента, типа пользователя и действия, а также ключевые измерения, такие как свойства пользователя (например, UserID), свойства расположения (например, IP-адрес клиента) и свойства продукта (например, ИД объекта).|
...
|[Схема Sway](#sway-schema)|Дополняет общую схему свойствами, характерными для всех событий Sway.|
...
|[Схема Dynamics 365](#dynamics-365-schema)|Расширяет общую схему свойствами, характерными для событий Dynamics 365.|
...
|[Схема обмена данными соответствия требованиям](#communication-compliance-exchange-schema)|Дополняет общую схему свойствами, характерными для нежелательной модели соответствия требованиям к общению.|
|||
...
|4|ExchangeItemGroup|События из журнала аудита почтовых ящиков Exchange для действий, которые выполняются с несколькими элементами, таких как перемещение или удаление одного либо нескольких электронных сообщений.|
...
|9 |AzureActiveDirectoryAccountLogon|События, связанные с входом в Azure Active Directory OrgId (прекращение поддержки).|
|10 |DataCenterSecurityCmdlet|События, связанные с командлетами для обеспечения безопасности центра обработки данных.|
|-11:00|ComplianceDLPSharePoint|События защиты от потери данных в SharePoint и OneDrive для бизнеса.|
|12 |Sway|События, связанные со службой и клиентами Sway.|
...
|двадцать|PowerBIAudit|События Power BI.|
|21|CRM|События Dynamics 365.|
|22|Yammer|События Yammer.|
|23|SkypeForBusinessCmdlets|События Skype для бизнеса.|
|открыт|Discovery|События для действий обнаружения электронных данных, выполняемых при поиске контента и управлении обращениями для обнаружения электронных данных в Центре безопасности и соответствия требованиям.|
...
|суммируемых|MailSubmission|События отправки из Exchange Online Protection и Office 365 Advanced Threat Protection.|
|более|MicrosoftFlow|События Microsoft Power Automate (ранее Microsoft Flow).|
|длиной|AeD|События Advanced eDiscovery.|
...
|68|комплианцесупервисионексчанже|События, отслеживаемые нежелательной моделью соответствия требованиям к связи.|
||||
...
|Значение|Имя элемента|Описание|
|:-----|:-----|:-----|
|нуль|Regular|Обычный пользователь.|
|1 |Reserved|Зарезервированный пользователь.|
|2|Admin|Администратор.|
|4|DcAdmin|Оператор центра данных Майкрософт.|
...
|Значение|Имя элемента|Описание|
|:-----|:-----|:-----|
|нуль|Online|Это событие создано в размещенной службе Office 365.|
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|Invalid|Элемент не относится ни к одному из других типов элементов (перечисленных в этой таблице).|
...
|-11:00|Page|Элемент представляет собой страницу.|
||||
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|SharePoint|Источник события — SharePoint.|
...
|AccessInvitationRevoked*|Администратор или владелец сайта либо документа в SharePoint или OneDrive для бизнеса отзывает приглашение, отправленное пользователю за пределами организации. Приглашение можно отозвать только до его принятия.|
...
|FileCheckOutDiscarded*|Пользователь удаляет или отменяет файл, полученный для изменения. Это значит, что любые изменения, которые он внес в файл при извлечении, будут отменены и не сохранены в версии документа, находящегося в библиотеке документов.|
...
> [!NOTE] 
> * Эта операция доступна в предварительной версии.
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|Owner|Владелец почтового ящика.|
...
|4|Transport|Служба транспорта в центре данных Майкрософт.|
...
|4|SilentReAuth|Вход с автоматической повторной проверкой подлинности.|
|||||
...
|-11:00|MD5|Способ проверки подлинности — MD5.|
...
|LogonError|Edm.String|Нет|Содержит сведения о причине неудачной попытки входа. Полное описание LogonErrors см. в списке [кодов ошибок проверки подлинности и авторизации](https://docs.microsoft.com/azure/active-directory/develop/reference-aadsts-error-codes#aadsts-error-codes).
|||||
...
- Все оповещения, создаваемые на основе [Политик оповещений в Центре безопасности и соответствия требованиям](https://docs.microsoft.com/office365/securitycompliance/alert-policies#default-alert-policies).
- Связанные с Office 365 оповещения, создаваемые в [Office 365 Cloud App Security](https://docs.microsoft.com/office365/securitycompliance/office-365-cas-overview) и [Microsoft Cloud App Security](https://docs.microsoft.com/cloud-app-security/what-is-cloud-app-security).

Параметры UserId и UserKey этих событий всегда относятся к элементу SecurityComplianceAlerts. Существует три типа событий оповещения, которые хранятся в виде значения свойства Operation общей схемы:

- AlertTriggered. Создается новое оповещение по причине соответствия политике.

- AlertEntityGenerated. Новая сущность добавляется в оповещение. Это событие применяется только к оповещениям, созданным на основе политик оповещений в центре безопасности и соответствия требованиям. Каждое созданное оповещение можно связать с одним или несколькими такими событиями. Например, политика оповещений настроена на срабатывание оповещения, если любой пользователь удаляет более 100 файлов в течение 5 минут. Если два пользователя превышают пороговое значение за то же время, возникает два события AlertEntityGenerated, но только одно событие AlertTriggered.

- Алертупдатед — в метаданные оповещения внесено обновление. Это событие записывается в журнал при изменении состояния оповещения (например, с "активно" на "разрешено") и при добавлении комментария к оповещению.
...
|Category|Edm.String|Нет|Категория оповещения. Категории: <ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p>DataLossPrevention</p></li><li><p>ThreatManagement</p></li><li><p>DataGovernance</p></li><li><p>AccessGovernance</p></li><li><p>MailFlow</p></li><li><p>Other</p></li></ul>|
...
Эта схема используется в событиях Yammer, перечисленных в статье [Поиск в журнале аудита в Центре безопасности и соответствия требованиям](https://docs.microsoft.com/microsoft-365/compliance/search-the-audit-log-in-security-and-compliance#yammer-activities).
...
|**Параметр**|**Тип**|**Обязательный?**|**Описание**|
|:-----|:-----|:-----|:-----|
|ObjectType|Self.[ObjectType](#objecttype)|Нет|Точка доступа для активированного события. К точкам доступа относятся: <ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p>клиент Sway;</p></li><li><p>клиент Sway, внедренный в узел;</p></li><li><p>параметры Sway на портале администрирования Office 365.</p></li></ul>|
|Endpoint|Self.[Endpoint](#endpoint)|Нет|Конечная точка клиента Sway для активированного события. К конечным точкам Sway относятся веб-клиент или клиент под управлением iOS, Windows либо Android. |
|BrowserName|Edm.String|Нет|Браузер, с которого получен доступ к Sway для активированного события. |
|DeviceType|Self.[DeviceType](#devicetype)|Нет|Тип устройства, с которого получен доступ к Sway для активированного события. Возможный тип устройства: настольный компьютер, мобильное устройство или планшет.|
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|Sway|Событие, активированное из клиента Sway.|
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|Succeeded|Событие успешно завершено.|
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|SwayWeb|Событие активировано с помощью веб-клиента Sway.|
...
|4|SwayAndroid|Событие активировано с помощью клиента Sway под управлением Android.|
|||||
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|Desktop|Событие активировано с использованием настольного компьютера.|
...
### <a name="enum-swayauditoperation---type-edmint32"></a>Enum: SwayAuditOperation; Type Edm.Int32

#### <a name="swayauditoperation"></a>SwayAuditOperation

|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|1,1|Create|Пользователь создает клиент Sway.|
|2|Delete|Пользователь удаляет клиент Sway.|
|4|Просмотреть|Пользователь просматривает клиент Sway.|
|4 |Edit|Пользователь изменяет клиент Sway.|
|5 |Duplicate|Пользователь дублирует клиент Sway.|
|7 |Share|Пользователь инициирует общий доступ к клиенту Sway. Это событие записывает действие пользователя, который щелкает определенное назначение для общего доступа в меню общего доступа Sway. Это событие не указывает, завершает ли пользователь действие по предоставлению общего доступа.|
|8 |ChangeShareLevel|Пользователь изменяет уровень общего доступа к клиенту Sway. Это событие записывает изменения, которые пользователь внес в уровень общего доступа к клиенту Sway. Например, такой доступ можно предоставить всем или только пользователям в пределах организации.|
|9 |RevokeShare|Пользователь отменяет общий доступ к клиенту Sway. При этом изменяются связи, ассоциируемые с клиентом Sway.|
|10 |EnableDuplication|Пользователь включает дублирование клиента Sway (включено по умолчанию).|
|-11:00|DisableDuplication|Пользователь отключает дублирование клиента Sway (отключено по умолчанию).|
|12 |ServiceOn|Пользователь включает клиент Sway для всей организации с помощью Центра администрирования Office 365 (включен по умолчанию).|
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|Member|Пользователь, который входит в группу.|
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|1,1|Bot|Программа-робот Microsoft Teams.|
|2|Connector|Соединитель Microsoft Teams.|
|4|Tab|Вкладка Microsoft Teams.|
||||
...
- Файл в SharePoint Online, OneDrive для бизнеса или Microsoft Teams, который был признан вредоносным защитой [Office 365 ATP](https://docs.microsoft.com/office365/securitycompliance/atp-for-spo-odb-and-teams).

- Активированное оповещение, запускающее [автоматизированный анализ угроз](https://docs.microsoft.com/office365/securitycompliance/automated-investigation-response-office).

> [!NOTE]
> Службы Office 365 Advanced Threat Protection и Office 365 Threat Investigation and Response (прежнее название — Office 365 Threat Intelligence) теперь входят в состав Office 365 Advanced Threat Protection (план 2) с дополнительными возможностями защиты от угроз. Дополнительные сведения см. на страницах [Планы и цены Office 365 ATP](https://products.office.com/exchange/advance-threat-protection) и [Описание службы Office 365 ATP](https://docs.microsoft.com/office365/servicedescriptions/office-365-advanced-threat-protection-service-description).
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|Good|Угрозы не обнаружены.|
...
|4|Anti-spam, Bulk|Групповое действие в политике защиты от нежелательной почты.|
...
|-11:00|Exchange transport rule, ETR| Действие политики в правиле транспорта Exchange.|
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|MoveToJMF|Действие политики состоит в перемещении в папку нежелательной почты.|
...
|4|Redirect|Действие политики состоит в переадресации сообщения электронной почты на адрес электронной почты, указанный в политике фильтрации.|
...
|4|PendingDetonationPage|Пользователю открывается ожидающая "детонации" страница на основании инструмента [Безопасные ссылки Office 365 ATP](https://docs.microsoft.com/office365/securitycompliance/atp-safe-links).|
...
- [Защита от исходящего спама](https://docs.microsoft.com/microsoft-365/security/office-365-security/outbound-spam-controls)

- [Удаление заблокированных пользователей с портала ограниченных пользователей в Office 365](https://docs.microsoft.com/microsoft-365/security/office-365-security/removing-user-from-restricted-users-portal-after-spam)
...
Записи аудита для событий, связанных с приложениями на основе моделей в событиях Dynamics 365, используют базовую схему и схему операции сущности. Дополнительные сведения см в разделе [Включение и использование журнала активности](https://docs.microsoft.com/power-platform/admin/enable-use-comprehensive-auditing#model-driven-apps-in-dynamics-365-schema).

### <a name="dynamics-365-base-schema"></a>Базовая схема Dynamics 365

| **Параметры**     | **Тип**            | **Обязательный?** | **Описание**|
|:------------------ | :------------------ | :--------------|:--------------|
|крморганизатионуникуенаме|Edm.String|Да|Уникальное имя Организации.|
|инстанцеурл|Edm.String|Да|URL-адрес экземпляра.|
|ItemUrl|Edm.String|Нет|URL-адрес записи, в которой создается журнал.|
|ItemType|Edm.String|Нет|Нааме объекта.|
|UserAgent|Edm.String|Нет|Уникальный идентификатор GUID пользователя в Организации.|
|Поля|Collection(Common.NameValuePair)|Нет|Объект JSON, содержащий пары "ключ — значение" свойства, которые были созданы или обновлены.|
|||||

### <a name="dynamics-365-entity-operation-schema"></a>Схема операции объекта Dynamics 365

События сущностей из приложений, управляемых моделями, в Dynamics 365 эта схема используется для создания в базовой схеме Dynamics 365. Эта схема включает сведения об операции с сущностью, которая вызвала событие, для которого ведется аудит.

| **Параметры**     | **Тип**            | **Обязательный?** | **Описание**|
|:------------------ | :------------------ | :--------------|:--------------|
|ентитид|Edm.Guid|Нет|Уникальный идентификатор объекта.|
|Имя_объекта|Edm.String|Да|Имя объекта в Организации. Пример сущностей include `contact` или `authentication` .|
|Message|Edm.String|Да|Этот параметр содержит операцию, которая была выполнена в связи с сущностью. Например, если создан новый контакт, значение свойства Message — `Create` и соответствующее значение свойства имя_объекта — `contact` .|
|Query|Edm.String|Нет|Параметры запроса фильтра, который использовался при выполнении операции Фетчксмл.|
|примарифиелдвалуе|Edm.String|Нет|Указывает значение для атрибута, который является основным полем для объекта.|
|||||
...
Эта схема используется в событиях карантина, перечисленных в статье [Поиск в журнале аудита в Центре безопасности и соответствия требованиям Office 365](https://docs.microsoft.com/microsoft-365/compliance/search-the-audit-log-in-security-and-compliance#quarantine-activities). Дополнительные сведения о карантине см. в разделе [Сообщения электронной почты на карантине в Office 365](https://docs.microsoft.com/microsoft-365/security/office-365-security/quarantine-email-messages).
...
|ReleaseTo|Edm.String|Нет|Адресат электронного сообщения.|
|||||
...
|**Значение**|**Имя элемента**|**Описание**|
|:-----|:-----|:-----|
|нуль|Предварительная версия|Это запрос пользователя на предварительный просмотр сообщения электронной почты, которое признано потенциально опасным.|
...
|4|Экспорт|Это запрос пользователя на экспорт сообщения электронной почты, которое признано потенциально опасным.|
...
|:-----|:-----|:-----|
|нуль|SCC|Центр безопасности и соответствия требованиям (SCC) — это источник, из которого может исходить запрос пользователя на предварительный просмотр, удаление, освобождение, экспорт или просмотр заголовка потенциально опасных сообщений электронной почты. |
...
|:-----|:-----|:-----|
|нуль|Администратор|Администратор —это лицо, имеющее доступ к форме.|
|1 |Владелец|Пользователь, являющийся владельцем формы.|
|2|Ответчик|Пользователь, отправивший ответ в форму.|
|4|Совместное редактирование|Пользователь, который использует ссылку для совместной работы, предоставленную владельцем формы для входа и редактирования формы.|
||||
...
|:-----|:-----|:-----|
|нуль|Форма|Формы, созданные при помощи параметра "Новая форма".|
...
Цель этой схемы аудита заключается в представлении сводки всех действий с электронной почтой, включающих метки конфиденциальности. Другими словами, для каждого сообщения электронной почты с примененной меткой, отправляемого или получаемого пользователям в организации, должно регистрироваться действие аудита, независимо от способа и времени применения метки конфиденциальности. Дополнительная информация о метках аудита:
...
## <a name="communication-compliance-exchange-schema"></a>Схема обмена данными соответствия требованиям

Эта схема используется в событиях соответствия требованиям на связь, перечисленных в журнале аудита Office 365. Сюда входят записи аудита для операции **супервисориревиеволаудит** , которая создается, когда содержимое сообщения электронной почты содержит оскорбительный язык, идентифицируемый моделями защиты от нежелательной почты с точностью до \> 99,5%.

|**Параметры**  |**Тип**|**Обязательный?** |**Описание**|
|:---------------|:-------|:--------------|:--------------|
| ексчанжедетаилс |[ексчанжедетаилс](#exchangedetails)|Нет|Свойства сообщения электронной почты, которое вызвало событие Супервисориревиеволаудит.|
|||||

### <a name="enum-exchangedetails---type-exchangedetails"></a>Enum: Ексчанжедетаилс — тип: Ексчанжедетаилс

#### <a name="exchangedetails"></a>ексчанжедетаилс

| **Имя элемента**   | **Тип**| **Описание**|
|:----------------- | :-------|:---------------|
| NetworkMessageId  |Edm.Guid|ИДЕНТИФИКАТОР сетевого сообщения электронного сообщения.|
| InternetMessageId |Edm.String|Идентификатор Интернет сообщения электронной почты.|
| AttachmentData|Коллекция ([AttachmentDetails](#attachmentdetails))|Сведения о файлах, вложенных в сообщение электронной почты.|
| Получатели|Collection(Edm.String)|Адреса электронной почты в полях "Кому", "копия" и "Скрытая копия" сообщения электронной почты. |
...
| From| Edm.String|Адрес электронной почты в поле отправителя сообщения электронной почты.|
| Письма|Edm.String|Состояние источника сообщения электронной почты.|
||||

### <a name="enum-attachmentdetails---type-edmint32"></a>Enum: AttachmentDetails — тип: EDM. Int32
