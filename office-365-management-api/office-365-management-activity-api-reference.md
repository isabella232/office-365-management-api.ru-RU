...
- <span data-ttu-id="d4b3d-122">DLP.All (только события DLP для всех рабочих нагрузок)</span><span class="sxs-lookup"><span data-stu-id="d4b3d-122">DLP.All (DLP events only for all workloads)</span></span>
...
## <a name="activity-api-operations"></a><span data-ttu-id="d4b3d-134">Операции с API действий</span><span class="sxs-lookup"><span data-stu-id="d4b3d-134">Activity API operations</span></span>
...
- <span data-ttu-id="d4b3d-145">**вывод списка доступного контента** и соответствующих URL-адресов;</span><span class="sxs-lookup"><span data-stu-id="d4b3d-145">**List available content** and the corresponding content URLs.</span></span>
...
|<span data-ttu-id="d4b3d-228">**Основной текст**</span><span class="sxs-lookup"><span data-stu-id="d4b3d-228">**Body**</span></span>|<span data-ttu-id="d4b3d-229">(пусто)</span><span class="sxs-lookup"><span data-stu-id="d4b3d-229">Empty</span></span>||
...
## <a name="notification-failure-and-retry"></a><span data-ttu-id="d4b3d-303">Сбой и повторная попытка отправки уведомления</span><span class="sxs-lookup"><span data-stu-id="d4b3d-303">Notification failure and retry</span></span>
...
## <a name="api-throttling"></a><span data-ttu-id="d4b3d-385">Регулирование API</span><span class="sxs-lookup"><span data-stu-id="d4b3d-385">API throttling</span></span>
...
<span data-ttu-id="d4b3d-392">Клиентам, разрабатывающим собственные решения с помощью этого API, мы рекомендуем использовать собственный GUID клиента во избежание конфликтов, вызванных ограничением общей квоты.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-392">For customers who are developing their own solutions using this API, we recommend using your own tenant GUID to avoid competition caused by a limited shared quota.</span></span>

> [!NOTE] 
> <span data-ttu-id="d4b3d-393">Несмотря на то что каждый издатель может отправлять до 60 тысяч запросов в минуту, корпорация Майкрософт не может гарантировать скорость отклика.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-393">Even though each publisher can submit up to 60K requests per minute, Microsoft cannot guarantee a response rate.</span></span> <span data-ttu-id="d4b3d-394">Она зависит от различных факторов, таких как производительность клиентской системы, а также мощность и скорость сети.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-394">The response rate depends on various factors, such as client system performance, network capacity, and network speed.</span></span>  <span data-ttu-id="d4b3d-395">Издатель может отправлять до 60 тысяч запросов в минуту, но не следует ожидать, что отклики на все 60 тысяч запросов придут в течение этой же минуты.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-395">A publisher can submit up to 60K requests per minute, but should not expect to receive responses for all 60K requests within that same minute.</span></span> <span data-ttu-id="d4b3d-396">На самом деле, если издатель хочет испытать клиентское приложение, это следует сделать в каждой среде, где планируется запускать это приложение, так как результаты в разных средах будут отличаться.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-396">If anything, should a publisher want to benchmark a client application, they should do so in each different environment that they are planning on running the client application in because results will vary from environment to environment.</span></span>

## <a name="errors"></a><span data-ttu-id="d4b3d-397">Ошибки</span><span class="sxs-lookup"><span data-stu-id="d4b3d-397">Errors</span></span>

<span data-ttu-id="d4b3d-398">Когда служба обнаруживает ошибку, она возвращает отправителю вызова соответствующий код отклика, используя стандартный синтаксис кодов ошибок HTTP.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-398">When the service encounters an error, it will report the error response code to the caller, using standard HTTP error-code syntax.</span></span> <span data-ttu-id="d4b3d-399">.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-399"></span></span> <span data-ttu-id="d4b3d-400">Дополнительные сведения включены в текст неудачного запроса в виде одного объекта JSON.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-400">Additional information is included in the body of the failed call as a single JSON object.</span></span> <span data-ttu-id="d4b3d-401">Ниже приведен пример полного текста ошибки JSON.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-401">An example of a full JSON error body is shown below:</span></span> 
...
|||
|:-----|:-----|
|<span data-ttu-id="d4b3d-402">Код</span><span class="sxs-lookup"><span data-stu-id="d4b3d-402">Code</span></span>|<span data-ttu-id="d4b3d-403">Сообщение</span><span class="sxs-lookup"><span data-stu-id="d4b3d-403">Message</span></span>|
|<span data-ttu-id="d4b3d-404">AF10001</span><span class="sxs-lookup"><span data-stu-id="d4b3d-404">AF10001</span></span>|<span data-ttu-id="d4b3d-405">Набор разрешений ({0}), отправленный в запросе, не включал ожидаемое разрешение **ActivityFeed.Read**.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-405">The permission set ({0}) sent in the request did not include the expected permission **ActivityFeed.Read**.</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-406">{0} — набор разрешений в маркере доступа.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-406">{0} = the permission set in the access token.</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-407">AF20001</span><span class="sxs-lookup"><span data-stu-id="d4b3d-407">AF20001</span></span>|<span data-ttu-id="d4b3d-408">Отсутствующий параметр: {0}.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-408">Missing parameter: {0}.</span></span> <ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-409">{0} — имя отсутствующего параметра.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-409">{0} = the name of the missing parameter.</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-410">AF20002</span><span class="sxs-lookup"><span data-stu-id="d4b3d-410">AF20002</span></span>|<span data-ttu-id="d4b3d-411">Недопустимый тип параметра: {0}.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-411">{0}</span></span> <span data-ttu-id="d4b3d-412">Ожидаемый тип: {1}</span><span class="sxs-lookup"><span data-stu-id="d4b3d-412">Expected type: {1}</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-413">{0} — имя недопустимого параметра.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-413">{0} = the name of the invalid parameter.</span></span></p></li><li><p><span data-ttu-id="d4b3d-414">{1} — ожидаемый тип (int, datetime, guid).</span><span class="sxs-lookup"><span data-stu-id="d4b3d-414">{1} = the expected type (int, datetime, guid).</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-415">AF20003</span><span class="sxs-lookup"><span data-stu-id="d4b3d-415">AF20003</span></span>|<span data-ttu-id="d4b3d-416">Время окончания срока действия ({0}) уже прошло.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-416">Expiration {0} provided is set to past date and time.</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-417">{0} — время окончания срока действия, переданное в вызове API.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-417">{0} = the expiration passed in the API call.</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-418">AF20010</span><span class="sxs-lookup"><span data-stu-id="d4b3d-418">AF20010</span></span>|<span data-ttu-id="d4b3d-419">ИД клиента, переданный в URL-адресе ({0}), не совпадает с ИД клиента в маркере доступа ({1}).</span><span class="sxs-lookup"><span data-stu-id="d4b3d-419">The tenant ID passed in the URL ({0}) does not match the tenant ID passed in the access token ({1}).</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-420">{0} — ИД клиента, переданный в URL-адресе.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-420">{0} = tenant ID passed in the URL</span></span></p></li><li><p><span data-ttu-id="d4b3d-421">{1} — ИД клиента, переданный в маркере доступа.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-421">{1} = tenant ID passed in the access token</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-422">AF20011</span><span class="sxs-lookup"><span data-stu-id="d4b3d-422">AF20011</span></span>|<span data-ttu-id="d4b3d-423">Указанный ИД клиента ({0}) не существует в системе или был удален.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-423">Specified tenant ID ({0}) does not exist in the system or has been deleted.</span></span> <ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p>   <span data-ttu-id="d4b3d-424">{0} — ИД клиента, переданный в URL-адресе.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-424">{0} = tenant ID passed in the URL</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-425">AF20012</span><span class="sxs-lookup"><span data-stu-id="d4b3d-425">AF20012</span></span>|<span data-ttu-id="d4b3d-426">Указанный ИД клиента ({0}) неправильно настроен в системе.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-426">Specified tenant ID ({0}) is incorrectly configured in the system.</span></span> <ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p>    <span data-ttu-id="d4b3d-427">{0} — ИД клиента, переданный в URL-адресе.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-427">{0} = tenant ID passed in the URL</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-428">AF20013</span><span class="sxs-lookup"><span data-stu-id="d4b3d-428">AF20013</span></span>|<span data-ttu-id="d4b3d-429">ИД клиента, переданный в URL-адресе ({0}), не является допустимым GUID.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-429">The tenant ID passed in the URL ({0}) is not a valid GUID.</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p> <span data-ttu-id="d4b3d-430">{0} — ИД клиента, переданный в URL-адресе.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-430">{0} = tenant ID passed in the URL</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-431">AF20020</span><span class="sxs-lookup"><span data-stu-id="d4b3d-431">AF20020</span></span>|<span data-ttu-id="d4b3d-432">Указанный тип контента не является допустимым.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-432">The aggregation type is not valid.</span></span>|
|<span data-ttu-id="d4b3d-433">AF20021</span><span class="sxs-lookup"><span data-stu-id="d4b3d-433">AF20021</span></span>|<span data-ttu-id="d4b3d-434">Не удалось проверить конечную точку веб-перехватчика {{0}).</span><span class="sxs-lookup"><span data-stu-id="d4b3d-434">The webhook endpoint {{0}) could not be validated.</span></span> <span data-ttu-id="d4b3d-435">{1}</span><span class="sxs-lookup"><span data-stu-id="d4b3d-435"></span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-436">{0} — адрес веб-перехватчика.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-436">{0} = webhook address.</span></span></p></li><li><p><span data-ttu-id="d4b3d-437">{1} — "Конечная точка не вернула код HTTP 200"</span><span class="sxs-lookup"><span data-stu-id="d4b3d-437">{1} = "The endpoint did not return HTTP 200."</span></span> <span data-ttu-id="d4b3d-438">или "Адрес должен начинаться с HTTPS".</span><span class="sxs-lookup"><span data-stu-id="d4b3d-438">or "The address must begin with HTTPS."</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-439">AF20022</span><span class="sxs-lookup"><span data-stu-id="d4b3d-439">AF20022</span></span>|<span data-ttu-id="d4b3d-440">Подписка для указанного типа контента не найдена.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-440">No subscription found for the specified content type.</span></span>|
|<span data-ttu-id="d4b3d-441">AF20023</span><span class="sxs-lookup"><span data-stu-id="d4b3d-441">AF20023</span></span>|<span data-ttu-id="d4b3d-442">Подписку отключил {0}.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-442">The subscription was disabled by {0}.</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-443">{0} — "администратор клиента" или "администратор службы"</span><span class="sxs-lookup"><span data-stu-id="d4b3d-443">{0} = "a tenant admin" or "a service admin"</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-444">AF20030</span><span class="sxs-lookup"><span data-stu-id="d4b3d-444">AF20030</span></span>|<span data-ttu-id="d4b3d-445">Необходимо указать время начала и окончания (или пропустить оба значения), а интервал между ними не должен превышать 24 часа. При этом со времени начала должно пройти не более 7 дней.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-445">Start time and end time must both be specified (or both omitted) and must be less than or equal to 24 hours apart, with the start time no more than 7 days in the past.</span></span>|
|<span data-ttu-id="d4b3d-446">AF20031</span><span class="sxs-lookup"><span data-stu-id="d4b3d-446">AF20031</span></span>|<span data-ttu-id="d4b3d-447">Недопустимые входные данные nextPage: {0}.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-447">Invalid nextPage Input: {0}.</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-448">{0} — индикатор следующей страницы, переданный в URL-адресе</span><span class="sxs-lookup"><span data-stu-id="d4b3d-448">{0} = the next page indicator passed in the URL</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-449">AF20050</span><span class="sxs-lookup"><span data-stu-id="d4b3d-449">AF20050</span></span>|<span data-ttu-id="d4b3d-450">Указанный контент ({0}) не существует.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-450">The specified Id does not exist.</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-451">{0} — идентификатор или URL-адрес ресурса</span><span class="sxs-lookup"><span data-stu-id="d4b3d-451">{0} = resource id or resource URL</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-452">AF20051</span><span class="sxs-lookup"><span data-stu-id="d4b3d-452">AF20051</span></span>|<span data-ttu-id="d4b3d-453">Контент, запрашиваемый с помощью ключа {0}, уже устарел.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-453">Content requested with the key {0} has already expired.</span></span> <span data-ttu-id="d4b3d-454">Контент старше 7 дней невозможно получить.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-454">Content older than 7 days cannot be retrieved.</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-455">• {0} — идентификатор или URL-адрес ресурса</span><span class="sxs-lookup"><span data-stu-id="d4b3d-455">•    {0} = resource id or resource URL</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-456">AF20052</span><span class="sxs-lookup"><span data-stu-id="d4b3d-456">AF20052</span></span>|<span data-ttu-id="d4b3d-457">Недопустимый ИД контента {0} в URL-адресе</span><span class="sxs-lookup"><span data-stu-id="d4b3d-457">Content ID {0} in the URL is invalid.</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-458">{0} — идентификатор или URL-адрес ресурса</span><span class="sxs-lookup"><span data-stu-id="d4b3d-458">{0} = resource id or resource URL</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-459">AF20053</span><span class="sxs-lookup"><span data-stu-id="d4b3d-459">AF20053</span></span>|<span data-ttu-id="d4b3d-460">В заголовке Accept-Language можно указать только один язык.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-460">Only one language may be present in the Accept-Language header.</span></span>|
|<span data-ttu-id="d4b3d-461">AF20054</span><span class="sxs-lookup"><span data-stu-id="d4b3d-461">AF20054</span></span>|<span data-ttu-id="d4b3d-462">Недопустимый синтаксис в заголовке Accept-Language.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-462">Invalid syntax in Accept-Language header.</span></span>|
|<span data-ttu-id="d4b3d-463">AF429</span><span class="sxs-lookup"><span data-stu-id="d4b3d-463">AF429</span></span>|<span data-ttu-id="d4b3d-464">Слишком много запросов.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-464">Too many requests.</span></span> <span data-ttu-id="d4b3d-465">Method={0}, PublisherId={1}</span><span class="sxs-lookup"><span data-stu-id="d4b3d-465">Method={0}, PublisherId={1}</span></span><ul xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:mtps="http://msdn2.microsoft.com/mtps" xmlns:mshelp="http://msdn.microsoft.com/mshelp" xmlns:ddue="http://ddue.schemas.microsoft.com/authoring/2003/5" xmlns:msxsl="urn:schemas-microsoft-com:xslt"><li><p><span data-ttu-id="d4b3d-466">{0} — метод HTTP</span><span class="sxs-lookup"><span data-stu-id="d4b3d-466">{0} = HTTP Method</span></span></p></li><li><p><span data-ttu-id="d4b3d-467">{1} — GUID клиента, используемый в качестве параметра PublisherIdentifier</span><span class="sxs-lookup"><span data-stu-id="d4b3d-467">{1} = Tenant GUID used as PublisherIdentifier</span></span></p></li></ul>|
|<span data-ttu-id="d4b3d-468">AF50000</span><span class="sxs-lookup"><span data-stu-id="d4b3d-468">AF50000</span></span>|<span data-ttu-id="d4b3d-469">Произошла внутренняя ошибка.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-469">An internal error has occurred.</span></span> <span data-ttu-id="d4b3d-470">Повторите запрос.</span><span class="sxs-lookup"><span data-stu-id="d4b3d-470">Retry the request.</span></span>|
