...
### <a name="use-the-azure-management-portal-to-register-your-application-in-azure-ad"></a>Регистрация приложения в Azure AD с помощью портала управления Azure

Подготовив клиент Майкрософт с подходящими подписками, вы можете зарегистрировать приложение в Azure AD.

1. Войдите на [портал управления Azure](https://manage.windowsazure.com/), используя учетные данные клиента, у которого есть нужная вам подписка на Office 365. Вы также можете получить доступ к порталу управления Azure по ссылке в левой области навигации на [портале администрирования Office](https://portal.office.com/).
    
2. В области навигации слева выберите Active Directory (1). Убедитесь, что выбрана вкладка "Каталог" (2), и выберите имя каталога (3).
    
   ![Страница регистрации Office 365](images/o365-sign-up-page.png)
    
    
3. На странице каталога выберите **Приложения**. В Azure AD появится список приложений, установленных в клиенте.
...
5. Нажмите **Добавить приложение, разрабатываемое моей организацией**.
    
6. Введите **имя** приложения и выберите **тип** "Веб-приложение и/или веб-API".
    
7. Укажите соответствующие свойства приложения:
    
   - **URL-адрес для входа.** URL-адрес, по которому пользователи могут входить и использовать приложение. При необходимости его можно изменить позже.
    
   - **URI кода приложения.** Универсальный код ресурса (URI), используемый в качестве уникального логического идентификатора приложения. URI должен относиться к проверенному личному домену внешнего пользователя, чтобы предоставить приложению доступ к его данным в Windows Azure AD. Например, если ваш клиент Майкрософт — **contoso.onmicrosoft.com**, то URI кода приложения будет **https://app.contoso.onmicrosoft.com**.
    
8. Теперь ваше приложение зарегистрировано в Azure AD, и ему назначен идентификатор клиента. Однако осталось настроить еще несколько важных аспектов приложения.
...
Дополнительные сведения о настройке приложений Azure AD в целом см. в статье [Объекты приложения и субъекта-службы в Azure Active Directory](https://docs.microsoft.com/ru-RU/azure/active-directory/develop/active-directory-application-objects).
...
1. На портале управления Azure выберите свое приложение и нажмите **Настройка** в верхнем меню. Прокрутите вниз до строки **ключи**.
    
2. Выберите длительность для ключа и нажмите кнопку **Сохранить**.
    
   ![Страница подписки Azure](images/azure-subscription-page.png)
    
    
3. Azure показывает секрет приложения только после его сохранения. Нажмите значок буфера обмена, чтобы скопировать секрет клиента.
    
   ![Страница портала Azure](images/azure-portal-page.png)

   > [!IMPORTANT] 
   > Azure показывает секрет клиента только в момент его создания. Вернуться на эту страницу и получить секрет клиента позже будет невозможно.
...
Дополнительные сведения см. в статье [Вызовы между службами с использованием учетных данных клиентов](https://msdn.microsoft.com/ru-RU/library/azure/dn645543.aspx).
...
6. На портале управления Azure выберите свое приложение и нажмите **Настройка** в верхнем меню.
    
7. На панели команд нажмите **Управление манифестом** и выберите команду **Скачать манифест**.
    
   ![Экран сертификата командной строки](images/command-line-certificate-display.png)
    
    
8. Откройте загруженный манифест для редактирования и замените пустое свойство KeyCredentials на следующий код JSON:
...
1. На портале управления Azure выберите свое приложение и нажмите **Настройка** в верхнем меню. Прокрутите вниз до раздела **Разрешения для других приложений** и нажмите **Добавить приложение**.
    
   ![Страница Azure AD](images/azure-ad-page.png)
    
    
2. Выберите **API управления Office 365** (1), чтобы этот пункт появился в столбце **Выбрано** (2), а затем установите флажок в правом нижней углу (3), чтобы сохранить выбор, и вернитесь на главную страницу конфигурации для приложения.
    
   ![Страница приложений Azure AD](images/azure-ad-apps-page.png)
    
    
3. Теперь API управления Office отображаются в списке приложений, для которого приложению требуются разрешения. В разделах **Разрешения приложения** и **Делегированные разрешения** выберите разрешения, необходимые приложению. Дополнительные сведения о каждом разрешении представлены в справочнике по определенному API.  

   > [!NOTE] 
   > В настоящее время существует четыре неиспользуемых разрешения, связанных с отчетами об активности и аналитикой угроз, которые будут удалены в будущем. Не выбирайте никакие из этих разрешений, так как в них нет необходимости.
...
- 
  [Поток предоставления кода авторизации](https://msdn.microsoft.com/en-us/library/azure/dn645542.aspx) включает предоставление явного согласия администратором клиента, в результате чего приложению возвращается код авторизации. Затем приложение обменивает код авторизации на маркер доступа. Этот способ необходим, чтобы получить первоначальное согласие, нужное приложению для доступа к данным клиента с помощью API, а этот первый маркер доступа необходим для получения и хранения идентификатора клиента.
    
- [Поток предоставления учетных данных клиента](https://msdn.microsoft.com/ru-RU/library/azure/dn645543.aspx) позволяет приложению запрашивать последующие маркеры доступа по мере устаревания предыдущих. При этом администратору клиента не требуется выполнять вход и явно предоставлять согласие. Этот способ необходимо использовать для приложений, непрерывно работающих в фоновом режиме, вызывая API после предоставления первоначального согласия администратором клиента.
...
Во время разработки приложения на платформе .NET можно создавать клиентские утверждения с помощью [библиотеки проверки подлинности Azure AD (ADAL)](https://docs.microsoft.com/ru-RU/azure/active-directory/develop/active-directory-authentication-libraries). На других платформах разработки должны быть доступны похожие библиотеки.
...
- **Управление маркерами доступа.** Вам потребуется компонент, который запрашивает и контролирует маркеры доступа по мере необходимости. Если приложение периодически вызывает интерфейсы API, оно может получать маркеры по требованию, а если оно непрерывно вызывает API для получения данных, можно запрашивать маркеры с определенным интервалом (например, каждые 45 минут).
